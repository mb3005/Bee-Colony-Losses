...
# 2015 starting colonies total 11681750

colony %>% 
  filter(State == 'United States')

not_US_2015Q1 <- colony %>% 
  filter((Year == '2015') & (!State == 'United States'))
...
# max colonies for the US for each year/quarter; output for 2015 matches previous test case: 14781830
US_max_colonies <- colony %>% 
  select(State, `Maximum Colonies`, Year, Quarter) %>% 
  filter(!State == "United States") %>% 
...
US_honey       # 6x8
```
```{r  Broad Glance: How have bee clonies overall changed over time in the US? }
US_colony %>% 
  ggplot(aes(x = Year, y = `Starting Colonies`)) + geom_line()
```
